  ...
  return<>
    ...
    <div className="stack">{kras.map((kra,ki)=><Card key={ki}>
      ...
      <div className="dynamic-kpi-list">{kra.items.map((item,ii)=>{
        ...
        const kpiWeight=kra.items.length>0?Number((Number(kra.weight||0)/kra.items.length).toFixed(2)):0
        return <div className="dynamic-kpi is-open" key={ii}>
          ...
          <div className="form-grid four">
            ...
            {!choice?<label>Unit<input value={item.unit} onChange={e=>updateItem(ki,ii,{unit:e.target.value})} placeholder="units / INR / % / days"/><span className="cell-help">Shown beside target, qualifier and employee input.</span></label>:<label>KPI weightage<input value={kpiWeight} disabled/><span className="cell-help">Every dropdown result maps to weightage out of 100.</span></label>}
            ...
            <label>KPI weightage<input value={kpiWeight} disabled/><span className="cell-help">KRA weight ÷ number of KPIs ({kra.weight} ÷ {kra.items.length}).</span></label>
