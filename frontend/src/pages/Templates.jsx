...
import {FileUp, Pencil, Plus, Trash2, Undo2} from 'lucide-react'
...
export default function Templates(){
  ...
  const divisionOptions=masters.map(d=>d.name).sort()
  const departmentOptions=[...new Set(masters.filter(d=>!divisionFilter||d.name===divisionFilter).flatMap(d=>d.departments.map(dep=>dep.name)))].sort()
  ...
  const visibleRows=(rows||[]).filter(t=>(!divisionFilter||t.division===divisionFilter)&&(!departmentFilter||t.department===departmentFilter))
  return <>
    <PageHeader title="KPI Templates" subtitle="Create easy task-based targets for each department and role." actions={<><button className="secondary" onClick={()=>setShowImport(v=>!v)}><FileUp size={16}/>Import</button><Link className="primary" to="/templates/new"><Plus size={16}/>Create simple template</Link></>}/>
    <ErrorBox error={error}/>{message?<div className="success-box">{message}</div>:null}
    <Card><div className="helper-strip"><strong>Hierarchy first:</strong> filter templates by division and department. Each template shows the exact hierarchy scope used for assignment.</div><div className="form-grid"><label>Show division<select value={divisionFilter} onChange={e=>{setDivisionFilter(e.target.value);if(e.target.value&&!masters.find(d=>d.name===e.target.value)?.departments.some(dep=>dep.name===departmentFilter))setDepartmentFilter('')}}><option value="">All divisions</option>{divisionOptions.map(x=><option key={x} value={x}>{x}</option>)}</select></label><label>Show department<select value={departmentFilter} onChange={e=>setDepartmentFilter(e.target.value)}><option value="">All departments</option>{departmentOptions.map(x=><option key={x} value={x}>{x}</option>)}</select></label></div></Card>
