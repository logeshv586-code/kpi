...
import {ExternalLink,FileUp,Info,Lightbulb} from 'lucide-react'
...
export default function KpiInput(){
  ...
  function setValue(itemId,patch){setValues(current=>({...current,[itemId]:{...current[itemId],...patch,kpi_item_id:itemId}}))}

  async function save(){setError('');setMessage('');try{const payload=allItems.map(i=>({kpi_item_id:i.id,actual_numeric:values[i.id]?.actual_numeric??null,answer_text:values[i.id]?.answer_text??null,selected_option:values[i.id]?.selected_option??null,remarks:values[i.id]?.remarks??null,evidence_url:null,evidence_file_id:values[i.id]?.evidence_file_id??null}));const {data}=await api.put(`/kpi/assignments/${id}/responses`,payload);setMessage(`Draft saved. Current calculated score: ${data.score}/100`);await loadAssignment();return true}catch(e){setError(getError(e));return false}}
