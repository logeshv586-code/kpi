...
export default function KpiInput(){
  ...
  return <>
    ...
    {!assignment?<Loader/>:<>
      ...
      <div className="stack">{assignment.template.kras.map(kra=>{const kraScore=kra.items.reduce((s,i)=>s+Number(i.response?.score||0),0);return <Card key={kra.id}><div className="kra-title"><div><h3>{kra.name}</h3><p>{kra.items.length} KPI parameters · enter actual achievement, measurement details, and attach PDF evidence</p></div><div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'4px'}}><div className="weight-chip">{kra.weight} marks weightage</div><div style={{fontSize:'0.85rem',fontWeight:700,color:'var(--color-primary,#2563eb)'}}>Section score: {kraScore.toFixed(1)} / {kra.weight} marks</div></div></div><div className="table-wrap"><table className="kpi-input-table"><thead><tr><th>KPI parameter & Task</th><th>Measurement / Target</th><th>Your result</th><th>Weight</th><th>Marks gathered</th><th>Measurement notes & PDF evidence</th></tr></thead><tbody>{kra.items.map(item=>{const meta=item.config?.meta||{},v=values[item.id]||{};return <tr key={item.id} style={{verticalAlign:'top'}}><td><strong>{item.question}</strong><Tooltip text={`Enter the actual result for this KPI. Input type: ${item.input_type}. ${item.direction==='lower'?'Lower values perform better.':'Higher values perform better.'}`}/>{meta.task_responsibility?<div style={{fontSize:'0.8rem',color:'var(--color-text-secondary,#475569)',marginTop:'2px'}}>{meta.task_responsibility}</div>:null}<div className="cell-help">{item.input_type.replaceAll('_',' ')} · {meta.frequency||'Monthly'}</div></td><td><div className="measurement-text">{meta.measurement||'Fill actual measurement details completed for the selected period.'}<div className="target-hint">Target: {item.target_value??(['choice','yesno'].includes(item.input_type)?'Choose an option':'Configured')}</div>{meta.evidence_required?<span className="required-chip">PDF required</span>:null}</div></td><td><div style={{minWidth:'140px'}}><Input disabled={locked} item={item} value={v} onChange={patch=>setValue(item.id,patch)}/></div></td><td><strong>{item.weight}</strong></td><td><strong style={{fontSize:'1.05rem',color:'var(--color-primary,#2563eb)'}}>{Number(item.response?.score||0).toFixed(1)}</strong><span style={{fontSize:'0.8rem',color:'var(--color-muted,#64748b)'}}> / {item.weight}</span><div className="cell-help">{Number(item.response?.achievement_pct||0).toFixed(0)}% score</div></td><td><div className="stack-tight" style={{display:'flex',flexDirection:'column',gap:'6px'}}><input disabled={locked} value={v.remarks||''} onChange={e=>setValue(item.id,{remarks:e.target.value})} placeholder="Write actual measurement notes or details completed..." style={{fontSize:'0.85rem'}}/><FileUpload compact disabled={locked} accept=".pdf" help="PDF only · maximum 10 MB" label="Upload PDF evidence" value={v.evidence_file||null} onUploaded={file=>setValue(item.id,{evidence_file_id:file?.file_id||null,evidence_file:file||null})}/>{v.evidence_file?<a className="evidence-link" href={apiFileUrl(v.evidence_file)} target="_blank" rel="noreferrer"><ExternalLink size={12}/>Open {v.evidence_file.filename}</a>:null}</div></td></tr>})}</tbody></table></div></Card>})}</div>
