...
function ResultSummary({item,value,prefix=''}){
  ...
  if(['choice','yesno'].includes(item.input_type)){
    if(!value?.[`${prefix}selected_option`])return <div className="cell-help">Choose one configured result option.</div>
    const pct=Number((cfg.score_map||{})[value[`${prefix}selected_option`]]||0)
    return <div className="kpi-threshold-summary"><div><strong>{value[`${prefix}selected_option`]}</strong> = {whole(pct)}% achievement</div></div>
  }
...
export default function KpiInputV2(){
  ...
  const liveScore=Math.min(100,whole(allItems.reduce((sum,item)=>sum+scoreItem(item,values[item.id]),0)))
  const liveManagerScore=Math.min(100,whole(allItems.reduce((sum,item)=>sum+scoreItem(item,values[item.id],'manager_'),0)))
  ...
  async function save(){if(!id||busy)return false;setBusy(true);setError('');setMessage('');try{const{data}=await api.put(`/kpi/assignments/${id}/responses`,payload());setMessage(`Draft saved. Score: ${whole(data.score)}/100`);await loadAssignment();loadList();return true}catch(e){setError(getError(e));return false}finally{setBusy(false)}}
  async function submit(){if(!ready){setError(`Complete all KPI results before submitting. ${missing.length} KPI parameter${missing.length===1?' is':'s are'} still missing.`);return}setBusy(true);setError('');setMessage('');try{const saved=await api.put(`/kpi/assignments/${id}/responses`,payload());const{data}=await api.post(`/kpi/assignments/${id}/submit`);setMessage(`Submitted successfully. Score: ${whole(data.score??saved.data.score??liveScore)}/100.`);await loadAssignment();loadList()}catch(e){setError(getError(e))}finally{setBusy(false)}}
  ...
  function applyImport(){if(!importPreview)return;setValues(current=>{const next={...current};importPreview.rows.forEach(row=>{if(!row.matched||!row.kpi_item_id)return;const item=allItems.find(i=>i.id===row.kpi_item_id);if(!item)return;next[item.id]={...next[item.id],kpi_item_id:item.id,remarks:row.remarks||next[item.id]?.remarks||''};if(['choice','yesno'].includes(item.input_type))next[item.id].selected_option=row.selected_option||next[item.id]?.selected_option||'';else if(row.actual_numeric!==null&&row.actual_numeric!==undefined)next[item.id].actual_numeric=row.actual_numeric});return next});setImportPreview(null);setShowImporter(false);setMessage('Imported achieved values applied. Review the calculated achievement, threshold status and marks before submitting.')}
  ...
  return <>
    <PageHeader title="KPI Input" subtitle="Enter only the achieved result. Target, threshold, achievement percentage and marks are calculated by the system." actions={<button className="secondary" disabled={!id} onClick={exportPdf}><Download size={16}/>Export PDF Report</button>}/>
    ...
    <div className="helper-strip"><strong>Published KPI:</strong> it can be filled on any day after publication. The 26th–1st window is only used for pending reminders and HR follow-up. If a configured minimum / maximum threshold is not met, that KPI is marked <strong>Not Achieved</strong> and receives 0 marks.</div>
    ...
    {!list?<Loader/>:list.length===0?<Card className="empty-assignment-card"><h3>No Active KPI Assignments Found</h3><p>No published KPI assignment exists for this account yet.</p>{isAdminOrHr?<button className="primary" disabled={busy} onClick={generateCycleAndLoad}>{busy?'Generating...':'Generate & Refresh KPI Assignments'}</button>:null}</Card>:!assignment?<Loader/>:<>
      ...
      <div className="metric-grid compact">
        ...
        <Card><span>Staff Score</span><strong className="small-metric">{liveScore}</strong><Status value={assignment.status}/></Card>
      </div>

      {!locked?<Card className={ready?'ready-card':'pending-card'}><div className="ready-row">{ready?<CheckCircle2 size={20}/>:<Info size={20}/>}<div><strong>{ready?'Ready to submit':'Complete KPI achieved values'}</strong><div className="cell-help">{ready?`All KPI results are filled. Current whole-number score ${liveScore}/100.`:`${missing.length} KPI result${missing.length===1?'':'s'} remaining.`}</div></div></div></Card>:null}
        ...
        return <Card key={kra.id}>
          ...
          <div className="table-wrap"><table className="kpi-input-table"><thead><tr><th>KPI parameter & task</th><th>Threshold criteria</th><th>Your achieved value</th><th>Manager achieved</th><th>Weight</th><th>Marks scored</th><th>Optional description & PDF</th></tr></thead><tbody>{kra.items.map(item=>{
            ...
            return <tr key={item.id}>
              ...
              <td><div><div className="cell-help">Staff</div><strong>{mark}</strong> / {item.weight}</div><div><div className="cell-help">Manager</div><strong>{mMark}</strong> / {item.weight}</div></td>
    ...
    {showMonths?<Modal title={`KPI Review Periods - ${people.find(p=>p.key===person)?.name||'Employee'}`} onClose={()=>setShowMonths(false)} className="wide-modal" actions={<button className="secondary" onClick={()=>setShowMonths(false)}>Close</button>}>
      ...
      <div className="period-card-grid">{personRows.filter(row=>String(row.month||'').startsWith(String(selectedYear))).map(row=>{
        ...
        return <div key={row.id} className={`calendar-month-card has-assignment ${active?'active-selected':''}`}><div className="calendar-month-header"><strong>{periodLabel(row)}</strong><Status value={row.status}/></div><div className="cell-help">{(row.review_type||'monthly').replace('_',' ')} · {row.financial_year||''}</div><div className="period-score">Score: {whole(row.final_score??row.manager_score??row.calculated_score)} / 100</div><div className="calendar-month-actions"><button type="button" className={active?'primary small':'secondary small'} onClick={()=>{selectPeriod(row.id);setShowMonths(false)}}><Eye size={13}/>View KPI</button><button type="button" className="secondary small" onClick={()=>downloadAssignmentPdf(row)}><Download size={13}/>PDF</button></div></div>
